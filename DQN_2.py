...
class DQNAgent(object):
    ...
    def __init__(self):
        ...
        self.model = self.network("weights_2.hdf5")
        self.epsilon = 0
        self.actual = []
        self.memory = []
...
(player.position[-1][-1] + 20) >= (game.game_height-20) or (player.position[-1][0]<=240 and player.position[-1][1] + 20 <= 120 and player.position[-1][1] + 20 >= 80) or (player.position[-1][0] == food.x_non_food and (player.position[-1][1] + 20) == food.y_non_food))),  # danger straight

            (player.x_change == 0 and player.y_change == -20 and ((list(map(add,player.position[-1],[20, 0])) in player.position) or
player.position[ -1][0] + 20 > (game.game_width-20) or (player.position[-1][1] == food.y_non_food and (player.position[-1][0] + 20) == food.x_non_food))) or (player.x_change == 0 and player.y_change == 20 and ((list(map(add,player.position[-1], [-20,0])) in player.position) or
player.position[-1][0] - 20 < 20 or (player.position[-1][1]>=80 and player.position[-1][1]<=120 and player.position[-1][0] - 20 <= 240) or (player.position[-1][1] == food.y_non_food and (player.position[-1][0] - 20) == food.x_non_food))) or (player.x_change == -20 and player.y_change == 0 and ((list(map(add,player.position[-1],[0,-20])) in player.position) or
player.position[-1][-1] - 20 < 20 or (player.position[-1][0]<=240 and player.position[-1][1] - 20 <= 120 and player.position[-1][1] - 20 >= 80) or (player.position[-1][0] == food.x_non_food and (player.position[-1][1] - 20) == food.y_non_food))) or (player.x_change == 20 and player.y_change == 0 and ((list(map(add,player.position[-1],[0,20])) in player.position) or
player.position[-1][-1] + 20 >= (game.game_height-20) or (player.position[-1][0]<=240 and player.position[-1][1] + 20 <= 120 and player.position[-1][1] + 20 >= 80) or (player.position[-1][0] == food.x_non_food and (player.position[-1][1] + 20) == food.y_non_food))),  # danger right
